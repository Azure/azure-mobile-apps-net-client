...
namespace MobileTest.Framework
{
    ...
    public static class Assert
    {
        ...
        {
            var extendedMessage = new StringBuilder(message);
            ...
            extendedMessage.AppendLine("Actual");
            for (int i = 0; i < first.Count(); i++)
            {
                extendedMessage.AppendLine(GetProperties(firstArray[i]));
            }

            var secondArray = second.ToArray();
            extendedMessage.AppendLine("Second");
            for (int i = 0; i < second.Count(); i++)
            {
                extendedMessage.AppendLine(GetProperties(secondArray[i]));
            }

            var intersectionArray = intersection.ToArray();
            extendedMessage.AppendLine("Expected");
            for (int i = 0; i < intersection.Count(); i++)
            {
                extendedMessage.AppendLine(GetProperties(intersectionArray[i]));
            }

            Assert.AreEqual(first.Count(), intersection.Count(), extendedMessage.ToString());
        }
        ...
        {
            var extendedMessage = new StringBuilder();
            extendedMessage.AppendLine();
            ...
            foreach (PropertyInfo propertyInfo in objType.GetRuntimeProperties())
            {
                if (propertyInfo.CanRead)
                {
                    ...
                    if (firstValue == null)
                        firstValue = "Null";
                    extendedMessage.AppendLine("Property Name:" + propertyInfo.Name.ToString() + ", Property Value:" + firstValue.ToString());
                }
            }
