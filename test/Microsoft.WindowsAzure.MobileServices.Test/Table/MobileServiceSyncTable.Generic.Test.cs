    ...
    public class MobileServiceSyncTable_Generic_Test
    {
        ...
        public async Task PullAsync_Cancels_WhenCancellationTokenIsCancelled()
        {
            ...
            using (var cts = new CancellationTokenSource())
            {
                ...
                var ex = await AssertEx.Throws<Exception>(() => pullTask);
        ...
        public async Task PullAsync_UpdatesDeltaToken_AfterEachResult_IfOrderByIsSupported()
        {
            ...
            await AssertEx.Throws<MobileServiceInvalidOperationException>(() => table.PullAsync("items", table.CreateQuery()));
            ...
            Assert.AreEqual(store.TableMap[MobileServiceLocalSystemTables.Config]["deltaToken|stringId_test_table|items"]["value"], "2001-02-04T00:00:00.0000000+00:00");
        }
        ...
        public async Task PullAsync_Throws_WhenPushThrows()
        {
            ...
            // this should trigger a push
            var ex = await AssertEx.Throws<MobileServicePushFailedException>(() => table.PullAsync(null, null));
        ...
        public async Task PullAsync_Throws_WhenSelectClauseIsSpecified()
        {
            ...
            var exception = await AssertEx.Throws<ArgumentException>(() => table.PullAsync(null, query, cancellationToken: CancellationToken.None));
            Assert.AreEqual(exception.ParamName, "query");
            StringAssert.StartsWith(exception.Message, "Pull query with select clause is not supported.");
        }
        ...
        public async Task PullAsync_Throws_WhenOrderByClauseIsSpecifiedWithQueryId()
        {
            ...
            var exception = await AssertEx.Throws<ArgumentException>(() => table.PullAsync("incQuery", query, cancellationToken: CancellationToken.None));
            Assert.AreEqual(exception.ParamName, "query");
            StringAssert.StartsWith(exception.Message, "Incremental pull query must not have orderby clause.");
        }
        ...
        public async Task PullAsync_Throws_WhenOrderByClauseIsSpecifiedAndOptionIsNotSet()
        {
            ...
            var exception = await AssertEx.Throws<ArgumentException>(() => table.PullAsync(null, query));
            Assert.AreEqual(exception.ParamName, "query");
            StringAssert.StartsWith(exception.Message, "The supported table options does not include orderby.");
        }
        ...
        public async Task PullAsync_Throws_WhenTopClauseIsSpecifiedAndOptionIsNotSet()
        {
            ...
            var exception = await AssertEx.Throws<ArgumentException>(() => table.PullAsync(null, query));
            Assert.AreEqual(exception.ParamName, "query");
            StringAssert.StartsWith(exception.Message, "The supported table options does not include top.");
        }
        ...
        public async Task PullAsync_Throws_WhenSkipClauseIsSpecifiedAndOptionIsNotSet()
        {
            ...
            var exception = await AssertEx.Throws<ArgumentException>(() => table.PullAsync(null, query));
            Assert.AreEqual(exception.ParamName, "query");
            StringAssert.StartsWith(exception.Message, "The supported table options does not include skip.");
        }
        ...
        public async Task PullAsync_Throws_WhenTopOrSkipIsSpecifiedWithQueryId()
        {
            ...
            var exception = await AssertEx.Throws<ArgumentException>(() => table.PullAsync("incQuery", query, cancellationToken: CancellationToken.None));
            Assert.AreEqual(exception.ParamName, "query");
            StringAssert.StartsWith(exception.Message, expectedError);

            query = table.Skip(5);
            exception = await AssertEx.Throws<ArgumentException>(() => table.PullAsync("incQuery", query, cancellationToken: CancellationToken.None));
            Assert.AreEqual(exception.ParamName, "query");
            StringAssert.StartsWith(exception.Message, expectedError);
        }
        ...
        public async Task PullAsync_Throws_WhenQueryIdIsInvalid()
        {
            ...
            await AssertEx.Throws<ArgumentException>(() => table.PullAsync("2as|df", table.CreateQuery(), CancellationToken.None));
        }
        ...
        private async Task TestPullQueryOverrideThrows(IDictionary<string, string> parameters, string errorMessage)
        {
            ...
            var ex = await AssertEx.Throws<ArgumentException>(() => table.PullAsync(null, query, cancellationToken: CancellationToken.None));
        ...
        public async Task PurgeAsync_Throws_WhenQueryIdIsInvalid()
        {
            ...
            await AssertEx.Throws<ArgumentException>(() => table.PurgeAsync("2as|df", table.CreateQuery(), CancellationToken.None));
        }
        ...
        public async Task PurgeAsync_Throws_WhenThereIsOperationInTable()
        {
            ...
            // this should trigger a push
            var ex = await AssertEx.Throws<InvalidOperationException>(() => table1.PurgeAsync());
        ...
        public async Task PurgeAsync_Throws_WhenThereIsOperationInTable_AndForceIsTrue_AndQueryIsSpecified()
        {
            ...
            // this should trigger a push
            var ex = await AssertEx.Throws<InvalidOperationException>(() => table.PurgeAsync(null, "$filter=a eq b", true, CancellationToken.None));
        ...
        public async Task DeleteAsync_Throws_WhenInsertWasAttempted()
        {
            ...
            var pushException = await AssertEx.Throws<MobileServicePushFailedException>(() => service.SyncContext.PushAsync());
            Assert.AreEqual(pushException.PushResult.Errors.Count(), 1);

            var delException = await AssertEx.Throws<InvalidOperationException>(() => table.DeleteAsync(item));
        ...
        private async Task TestCollapseThrow(Func<IMobileServiceSyncTable<StringIdType>, StringIdType, Task> firstOperation, Func<IMobileServiceSyncTable<StringIdType>, StringIdType, Task> secondOperation)
        {
            ...
            await AssertEx.Throws<InvalidOperationException>(() => secondOperation(table, item));
        ...
        private async Task TestPushAbort(Exception toThrow, MobileServicePushStatus expectedStatus)
        {
            ...
            MobileServicePushFailedException ex = await AssertEx.Throws<MobileServicePushFailedException>(service.SyncContext.PushAsync);
