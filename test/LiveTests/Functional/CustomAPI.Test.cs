...
using MobileTest.Framework;
using MobileTest.LiveTests.Base;
using MobileTest.LiveTests.TestData;
using Newtonsoft.Json.Linq;

namespace MobileTest.LiveTests.Functional
{
    [Tag("CustomAPI")]
    public class CustomAPITests : FunctionalTestBase
    {
        ...
        {
            // This test assumes the runtime will already have Movies data matching QueryTestData.cs
        ...
        private static void ValidateResponseHeader(HttpStatusCode expectedStatus, Dictionary<string, string> expectedHeaders, HttpResponseMessage response)
        {
            ...
            foreach (var reqHeader in expectedHeaders.Keys)
            {
                ...
                {
                    Assert.Fail("Error, expected header " + reqHeader + " not found");
                    break;
                }
                else
                {
                    Assert.AreEqual(expectedHeaders[reqHeader], headerValue.FirstOrDefault());
                }
            }
        }
